...
gl.uniform4f(colorLocation, 0, 0, 1, 1); // Azul como cor padrão
...
let mode = "line"; // Modo inicial para traçar retas
let points = [];
let clickCount = 0;
let currentColorMode = false;
let currentThicknessMode = false;
let thickness = 1; // Espessura inicial
...
function render() {
    ...
    gl.lineWidth(thickness); // Define a espessura do traçado
...
document.addEventListener("keydown", (event) => {
    if (event.key === "r" || event.key === "R") {
        ...
        currentThicknessMode = false;
    } else if (event.key === "e" || event.key === "E") {
        currentThicknessMode = true;
        currentColorMode = false;
    } else if (currentColorMode && !isNaN(event.key)) {
        const colors = [
            [1, 0, 0, 1], // vermelho
            [0, 1, 0, 1], // verde
            [0, 0, 1, 1], // azul
        ...
        render();
    } else if (currentThicknessMode && event.key >= "1" && event.key <= "9") {
        thickness = parseInt(event.key);
        render();
    }
});